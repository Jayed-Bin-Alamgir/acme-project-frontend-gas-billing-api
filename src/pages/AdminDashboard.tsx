...
export default function AdminDashboard() {
  const [config, setConfig] = useState({
    rate_per_litre: 0, // Changed name to match Backend Entity
    vat_percentage: 0,
    service_charge: 0,
    admin_pin: ''
  });
  ...
  useEffect(() => {
    fetch('http://localhost:3000/billing/config')
      .then((res) => res.json())
      .then((data) => {
        setConfig({
          ...
          admin_pin: '' // Don't pre-fill the PIN for security
        });
        setLoading(false);
      })
      .catch((err) => {
...
const handleUpdate = async (e: React.FormEvent) => {
  e.preventDefault();
  try {
    const response = await fetch('http://localhost:3000/billing/config', {
      method: 'PATCH',
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
              ...
              <div className="pt-6 border-t border-slate-100">
                ...
                <input 
                  ...
                  className="w-full max-w-md p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-500 outline-none bg-slate-50"
